...
import { disposeGroup, removeDrawerBtn, hideDrawerTabForGroup } from "./dockview-group.js"
...
DockviewComponent.prototype.removePanel = function (...args) {
    ...
    if (!panel.group.locked) {
        const closeMode = panel?.params?.closeMode?.toString()?.toLowerCase();
        const isDrawer = panel?.group?.getParams?.()?.floatType === 'drawer';
        if (closeMode === 'hidedrawertab' && isDrawer && hideDrawerTabForGroup(panel.group)) {
            return;
        }
