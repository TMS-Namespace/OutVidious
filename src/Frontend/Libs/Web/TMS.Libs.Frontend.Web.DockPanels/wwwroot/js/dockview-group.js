...
const addPanelWidthGroupId = (dockview, panel, index) => {
    ...
    // console.log('[DockView] addPanelWidthGroupId checking group:', panel.groupId, 'found:', !!group);
    let { rect = {}, packup, floatType, drawer, direction = 'left' } = panel.params || {}
    if (!group) {
        // console.log('[DockView] creating new group for panel:', panel.title, 'groupId:', panel.groupId);
...
const addPanelWidthCreatGroup = (dockview, panel, panels) => {
    const findGridGroupForPanel = (dockviewInstance, panelRef) => {
        return dockviewInstance.groups.find(group =>
            group.model?.location?.type === 'grid' && findContentFromPanels(group.panels, panelRef));
    };
    ...
    if (brothers.length > 0 && brothers[0].params.parentType == 'group') {
        group = findGridGroupForPanel(dockview, brothers[0])
    }
    else {
        let targetPanel
        for (let i = 0, len = panels.length; i < len; i++) {
            if (panels[i]?.id === panel.id) {
                if (i == len - 1) {
                    ...
                    group = findGridGroupForPanel(dockview, targetPanel)
                    if (group) {
    ...
    if (!group) {
        group = dockview.groups.find(g => g.model?.location?.type === 'grid' && g.panels.length > 0)
        if (group && !direction) {
            direction = 'right'
        }
    }
...
const closeModeValues = {
    hideDrawerTab: 'hidedrawertab'
}

const getCloseMode = group => {
    const panel = group.activePanel || group.panels?.[0];
    const closeMode = panel?.params?.closeMode;
    return typeof closeMode === 'string' ? closeMode.toLowerCase() : '';
}

const hideDrawerTabForGroup = group => {
    const dockview = group.api.accessor
    const params = group.getParams?.() || {}
    ...
    const button = dockview.element.querySelector(`.bb-dockview-aside>[groupId="${groupId}"]`)
    if (button) {
...
close = group => {
    ...
    const closeMode = getCloseMode(group)
    if (closeMode === closeModeValues.hideDrawerTab && hideDrawerTabForGroup(group)) {
        return
    }
