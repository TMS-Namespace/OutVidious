...
/// </summary>
public partial class NativeVideoPlayerComponent : ComponentBase, IPlayer, IAsyncDisposable
{
    ...
    private DotNetObjectReference<NativeVideoPlayerComponent>? _dotNetRef;
    ...
    private ILogger<NativeVideoPlayerComponent> Logger { get; set; } = NullLogger<NativeVideoPlayerComponent>.Instance;
    ...
    public VideoPlayerVariant Variant => VideoPlayerVariant.Native;

    public PlayerCapabilities Capabilities => PlayerCapabilities.Native;

    private string VideoElementId => $"native-video-{_instanceId}";

    private IReadOnlyList<CaptionTrack> AvailableCaptionTracks => Captions
        .Where(caption => !string.IsNullOrWhiteSpace(caption.Url))
        .ToList();
    ...
    protected override void OnParametersSet()
    {
        if (!ReferenceEquals(_lastLoggerFactory, LoggerFactory))
        {
            ...
            Logger = (LoggerFactory ?? NullLoggerFactory.Instance).CreateLogger<NativeVideoPlayerComponent>();
        }

        if (!string.Equals(_lastStreamUrl, StreamUrl, StringComparison.Ordinal))
        {
        ...
        if (!string.Equals(_lastCaptionId, SelectedCaptionId, StringComparison.Ordinal) ||
    ...
    private async Task RegisterAsync(CancellationToken cancellationToken)
    {
        ...
        catch (JSException ex)
        {
            Logger.LogError(
                ex,
                "[{MethodName}] Unexpected error: Failed to register native player element '{ElementId}'.",
    ...
    {
        Logger.LogDebug(
            "[{MethodName}] Quality selection is not supported in native mode.",
    ...
    {
        return await InvokeJsBooleanAsync(
            CombinedVideoPlayerJsInteropConstants.Reload,
            cancellationToken,
            VideoElementId);
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (_jsModule is not null)
        {
            ...
            catch (JSException ex)
            {
                Logger.LogError(
                    ex,
                    "[{MethodName}] Unexpected error: Failed to unregister native player '{ElementId}'.",
                    nameof(DisposeAsync),
                    VideoElementId);
            }
