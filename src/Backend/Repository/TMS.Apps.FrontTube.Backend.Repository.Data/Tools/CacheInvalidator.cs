...
internal class CacheInvalidator
{
        ...
        ILoggerFactory loggerFactory)
    {
        ...
        _logger = loggerFactory.CreateLogger<CacheInvalidator>();
    }

    public async Task<
        IReadOnlyList<(
            CacheResult<TEntity> CacheResult,
            IReadOnlyList<CacheResult<CacheImageEntity>> ImagesCacheResults,
        ...
        where TMapingEntity : class, IImageMap
    {
        var goodParentsCacheResultWithImages = parentsCacheResultsWithCommonImages
            .Where(r => r.CacheResult.Status != EntityCacheStatus.Error);

        if (!goodParentsCacheResultWithImages.Any()) // all parents have errors
        {
            return parentsCacheResultsWithCommonImages.Select(r =>
                (r.CacheResult,
                (IReadOnlyList<CacheResult<CacheImageEntity>>)[], (IReadOnlyList<TMapingEntity>)[]))
                .ToList();
        }
        ...
        var cacheResultsWithImagesAndMaps = parentsCacheResultsWithCommonImages
            .Where(r =>
                r.CacheResult.Status == EntityCacheStatus.Error)
            .Select(r =>
                    (r.CacheResult,
                    (IReadOnlyList<CacheResult<CacheImageEntity>>)[], (IReadOnlyList<TMapingEntity>)[])
                )
        ...
        var imagesCacheResults = await InvalidateCachedAsync<CacheImageEntity>(
            allGoodImagesCommon,
            dataBaseContext,
            cancellationToken,
            null);
        ...
        foreach (var goodParentCacheResultWithImages in goodParentsCacheResultWithImages)
        {
            ...
            var parentNewImagesCacheResults = imagesCacheResults
                ...
                .Where(ci => ci.Status == EntityCacheStatus.New)
                .ToList();
        ...
        where T : class, ICacheableEntity
    {
        ...
        var newCacheResults = cacheResults
            .Where(r => r.Status == EntityCacheStatus.New);
    ...
    public void DuplicateCheck(IEnumerable<(IEntity Entity, EntityCacheStatus Status)> entitiesWithStatuses, string caption)
    {
        ...
        if (cacheableEntitiesHashes.Count() != cacheableEntitiesHashes.Distinct().Count())
        {
            ...
            foreach (var duplicateHash in duplicateHashes)
            {
                ...
                foreach (var entity in duplicateEntities)
                {
                    ...
                    if (entity is CacheImageEntity imageEntity)
                    {
        ...
        for (int i = 0; i < commonsWithIdentity.Count; i++)
        {
            ...
            switch (commonWithIdentity.Common)
            {
                case VideoCommon common:
                    result = CommonToEntityMapper.ToEntity(common, (CacheVideoEntity?)existingEntity);
                    break;
                case VideoMetadataCommon common:
                    result = CommonToEntityMapper.ToEntity(common, (CacheVideoEntity?)existingEntity);
                    break;
                case ChannelCommon common:
                    result = CommonToEntityMapper.ToEntity(common, (CacheChannelEntity?)existingEntity);
                    break;
                case ChannelMetadataCommon common:
                    result = CommonToEntityMapper.ToEntity(common, (CacheChannelEntity?)existingEntity);
                    break;
                case ImageMetadataCommon common:
                    result = CommonToEntityMapper.ToEntity(common, (CacheImageEntity?)existingEntity);
                    break;
                case CaptionMetadataCommon common:
                    result = CommonToEntityMapper.ToEntity(common, (CacheCaptionEntity?)existingEntity);
                    break;
                case StreamMetadataCommon common:
                    result = CommonToEntityMapper.ToEntity(common, (CacheStreamEntity?)existingEntity);
                    break;
        ...
        where T : class, ICacheableEntity
    {
        ...
        if (typeof(T) == typeof(CacheVideoEntity))
        {
        ...
        if (typeof(T) == typeof(CacheChannelEntity))
        {
            var entities = await dbContext
        ...
        if (typeof(T) == typeof(CacheImageEntity))
        {
        ...
        if (typeof(T) == typeof(CacheCaptionEntity))
        {
        ...
        if (typeof(T) == typeof(CacheStreamEntity))
        {
        ...
        where T : class, ICacheableEntity
    {
        ...
        (
            Repository.Enums.EntityCacheStatus.Existed,
        ...
        (
            Repository.Enums.EntityCacheStatus.Updated,
            ue.Identity,
            (T)ue.Entity,
            ue.Common,
            null)));
        ...
        (
            Repository.Enums.EntityCacheStatus.New,
            ce.Identity,
            (T)ce.Entity,
            ce.Common,
            null)));
